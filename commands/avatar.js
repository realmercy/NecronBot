...
if (user) { const embed = new MessageEmbed() .setImage(`${user.displayAvatarURL({ dynamic: true, size: 4096 })}`) 
...
.setURL( `${interaction.user.displayAvatarURL({ dynamic: true, size: 4096 })}` ); 
return interaction.reply({ embeds: [embed] }); } },};
